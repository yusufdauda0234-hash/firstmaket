...
Sprint 8 (Cart and Multi-Vendor Checkout) is complete. A persistent **cart** (`/cart`) holds items from any vendor with a per-item quantity, re-validating Approved status and stock on every mutation. Checkout has two branches, split on **when payment happens**: **pay in full** (`/cart/checkout`) collects the delivery address upfront on the checkout screen, debits the wallet once for the whole cart total, and creates one `Order` per unit â€” possibly across several vendors â€” grouped by a `checkout_sessions` row for "placed together" receipts; a quantity of *N* fans out into *N* separate orders at the unit price, exactly like the existing one-order-per-unit model. **Pay Small** instead sends selected cart items into a Product Target Plan: a single item reuses the existing StartPlan flow unchanged, while two or more selected items â€” possibly from different vendors â€” bundle into one **multi-product plan** (`plan_items`, `product_target_plans.product_id` now nullable) with one combined target and one contribution schedule, gated by a new swappable `PlanEligibilityContract` (`RuleBasedPlanEligibilityChecker`: account â‰¥30 days old, â‰¥1 completed plan or â‰¥2 delivered Pay At Once orders, â‰¤1 currently cancelled plan â€” Sprint 9 swaps this for an AI-scored implementation behind the same contract). A bundled plan still asks for the delivery address only once fully funded, exactly like a single-product plan, but reaching 100% then creates **one order per bundled product in a single transaction** â€” never a subset early. Every resulting order, from either branch, runs through the unmodified Sprint 6 fulfillment chain with no vendor ever seeing another vendor's items in the same checkout or plan.

Sprint 9 (AI, Reporting, and Operational Controls) is complete. An audit before building found vendor suspendâ†’auto-delist and suspended/banned session revocation already working from earlier sessions, so neither was rebuilt â€” new work: a **Listing Review Assistant** (`AnalyzeListingJob`, queued automatically whenever a product is submitted or re-submitted after a price change) writes an advisory `ai_listing_reviews` row the approval queue displays â€” flags for price outliers (vs. the category average, threshold configurable at `/settings/ai`), thin descriptions, low image counts, and a prohibited-phrase scan; it never approves, rejects, or otherwise touches a product's status, and a failed analysis still records an "needs manual review" row rather than silently producing nothing. Both this and the Sprint 8 multi-product plan eligibility check (`PlanEligibilityContract` â†’ `AiScoredPlanEligibilityChecker`) are built behind swappable, provider-agnostic contracts with **no real AI provider wired in yet** â€” `RuleBasedListingAnalyzer` and `RuleBasedPlanEligibilityChecker` are the zero-cost defaults, ready to be swapped for a real driver later without touching the job, admin UI, or call sites. New **user moderation**: Administrators can suspend or ban a customer account (`/customers`, reason required, staff accounts and self-moderation refused server-side) â€” the account's session ends on its very next request via the existing `EnsureUserIsActive` enforcement. New **reports dashboard** (`/reports`): signups, deposits, plan completions, order volume, vendor activity, and product approval outcomes for an admin-chosen date range, each a live read straight from its source table (no snapshot/cache) with a CSV export. Vendor suspension now previews how many approved products the action will delist before an admin confirms.
