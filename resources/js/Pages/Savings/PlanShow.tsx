...
interface Props {
    plan: {
        ...
        currentProductPriceKobo: number;
...
const sourceLabel: Record<string, string> = {
    paystack_deposit: 'From wallet',
    open_savings: 'From Open Savings',
    redirection: 'Redirected balance',
};

export default function PlanShow() {
    const { plan, walletBalanceKobo, openSavingsBalanceKobo, orderUuid } = usePage<Props>().props;
    ...
    const priceDropped = plan.currentProductPriceKobo < plan.targetPriceKobo;
    const priceRose = plan.currentProductPriceKobo > plan.targetPriceKobo;

    return (
        <AccountLayout title="Product Tracker">
            <Head title={`Plan — ${plan.productName}`} />
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-900 p-6 text-white shadow-lg sm:p-7">
                <div className="relative z-[1] flex flex-wrap items-start gap-4">
                    <span className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white/10 ring-2 ring-white/30">
                        {plan.productImage ? (
                    ...
                    <div className="min-w-0 flex-1">
                        <Link
                            href={route('catalog.product', plan.productSlug)}
                            className="block truncate text-lg font-extrabold tracking-tight hover:underline"
                        >
                            {plan.productName}
                        </Link>
                        <p className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-brand-100">
                            <span className="inline-flex items-center gap-1">
                                <Lock className="h-3.5 w-3.5" /> Locked at {formatNairaFromKobo(plan.targetPriceKobo)}
                            </span>
                        ...
                        {priceDropped && (
                            <p className="mt-1 text-xs text-brand-100">
                                Product now sells for {formatNairaFromKobo(plan.currentProductPriceKobo)} — your locked
                                price stays.
                            </p>
                        )}
                        {priceRose && (
                            <p className="mt-1 text-xs font-semibold text-brand-yellow">
                                Product now sells for {formatNairaFromKobo(plan.currentProductPriceKobo)} — you locked
                                it cheaper. 🎉
                            </p>
                        )}
                    </div>
            ...
            {/* ── Ready for delivery: track existing order or capture address ── */}
            {orderUuid !== null && (
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4">
                    <p className="flex items-center gap-2 text-sm text-emerald-900">
                        ...
                        Your order is placed and moving through delivery.
                    </p>
                    <Link
                        href={route('orders.show', orderUuid)}
            ...
            {isReady && orderUuid === null && (
                <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
                    ...
                    <p className="mt-1 text-sm text-emerald-800">
                        ...
                        Add your delivery address to place the order — the vendor is notified immediately.
                    </p>
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                    ...
                    >
                        ...
                        <div className="sm:col-span-2">
                            ...
                            >
                                {addressForm.processing ? 'Placing order…' : 'Place my order'}
                            </button>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_320px]">
                    ...
                    <Card className="p-0">
                        ...
                        {plan.contributions.length === 0 ? (
                            <p className="px-5 py-8 text-center text-sm text-gray-400">
                                No contributions yet — every naira you add shows here.
                            </p>
                        ) : (
                            <ul className="divide-y divide-gray-100">
                                {plan.contributions.map((contribution) => (
                                    <li key={contribution.id} className="flex items-center gap-3 px-5 py-3.5">
                                        <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600">
                                            <CheckCircle2 className="h-[18px] w-[18px]" />
                                        </span>
                                        <div className="min-w-0 flex-1">
                                            <p className="text-sm font-medium text-gray-900">
                                                {sourceLabel[contribution.source] ?? contribution.source}
                                            </p>
                                            <p className="text-xs text-gray-400">{contribution.date}</p>
                                        </div>
                                        <p className="text-sm font-bold text-emerald-600">
                                            + {formatNairaFromKobo(contribution.amountKobo)}
                                        </p>
                                    </li>
                                ))}
                            </ul>
                ...
                <div className="space-y-3">
                    {isActive && openSavingsBalanceKobo > 0 && (
                        <Card>
                            <h3 className="flex items-center gap-2 text-sm font-bold text-gray-900">
                                <ArrowUpRight className="h-4 w-4 text-brand-600" /> Redirect Open Savings
                            </h3>
                            <p className="mt-1 text-xs text-gray-500">
                                Move your full Open Savings balance ({formatNairaFromKobo(openSavingsBalanceKobo)})
                                into this plan in one go.
                            </p>
                            {confirmRedirect ? (
                                <div className="mt-3 flex gap-2">
                                    <button
                                        ...
                                        onClick={() =>
                                            actionForm.post(route('savings.plans.redirect-open-savings', plan.uuid), {
                                                preserveScroll: true,
                                                onFinish: () => setConfirmRedirect(false),
                                            })
                                        }
                                        className="flex-1 rounded-full bg-brand-600 py-2 text-sm font-bold text-white transition hover:bg-brand-700 active:scale-95 disabled:opacity-60"
                                    >
                                        Yes, redirect
                                    </button>
                                    <button
                                ...
                                <button
                                    ...
                                    className="mt-3 w-full rounded-full border border-brand-200 bg-brand-50 py-2 text-sm font-bold text-brand-700 transition hover:bg-brand-100 active:scale-95"
                                >
                                    Redirect into this plan
                                </button>
                            )}
                        </Card>
                    )}
                    ...
                    {(isActive || isPaused) && (
                        <Card>
                            ...
                            <p className="mt-1 text-xs text-gray-500">
                                Pausing keeps your money locked to this target and your price unchanged.
                            </p>
                            {isActive ? (
                                <button
                                    type="button"
                    ...
                    {isActive && (
                        <Card>
                            <h3 className="text-sm font-bold text-gray-900">Changed your mind?</h3>
                            <p className="mt-1 text-xs text-gray-500">
                                Switch this plan to a different product — your full balance carries over and the new
                                price locks fresh. Find the product in the catalog and choose “Save Small Small”, or
